...
def sample_app() -> click.Group:
    ...
    @click.pass_context
    def app(ctx: click.Context, verbose: bool, color: str) -> None:
    ...
    @click.option('--format', type=click.Choice(['text', 'json', 'yaml']), default='text',
                 help='Output format')
    @click.pass_context
    def output(ctx: click.Context, format: str) -> None:
        """Output commands."""
        ctx.ensure_object(dict)
...
def test_app_with_config(sample_app: click.Group, config_file: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @click.option('--color', type=click.Choice(['auto', 'always', 'never']), default='auto',
                 help='Colorize output')
    @click.pass_context
    def app_with_config(ctx: click.Context, verbose: bool, color: str) -> None:
        """Sample application with configuration."""
    ...
    # Add subcommands
    for name, cmd in sample_app.commands.items():
        app_with_config.add_command(cmd)
